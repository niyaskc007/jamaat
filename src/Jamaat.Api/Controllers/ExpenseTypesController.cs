...
public sealed class ExpenseTypesController(IExpenseTypeService svc) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = "admin.masterdata")]
    public async Task<IActionResult> List([FromQuery] ExpenseTypeListQuery q, CancellationToken ct) => Ok(await svc.ListAsync(q, ct));
    ...
    [HttpPost]
    [Authorize(Policy = "admin.masterdata")]
    public async Task<IActionResult> Create([FromBody] CreateExpenseTypeDto dto, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    [Authorize(Policy = "admin.masterdata")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateExpenseTypeDto dto, CancellationToken ct)
    {
