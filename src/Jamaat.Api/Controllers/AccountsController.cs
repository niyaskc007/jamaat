...
public sealed class AccountsController(IAccountService svc) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = "admin.masterdata")]
    public async Task<IActionResult> List([FromQuery] AccountListQuery q, CancellationToken ct) => Ok(await svc.ListAsync(q, ct));

    [HttpGet("tree")]
    [Authorize(Policy = "admin.masterdata")]
    public async Task<IActionResult> Tree(CancellationToken ct) => Ok(await svc.TreeAsync(ct));
