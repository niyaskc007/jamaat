    ...
    public async Task<PagedResult<EventScanDto>> ListScansAsync(ScanListQuery q, CancellationToken ct = default)
    {
        ...
        var items = await query.OrderByDescending(x => x.ScannedAtUtc)
            ...
            .Select(x => new EventScanDto(
                ...
                x.MemberId,
                // Value-converted ItsNumber column; `.Value` reads the underlying string
                // through the converter and EF Core 10 translates this correctly when the
                // outer query is itself a Select projection.
